...
const RULES = [
  ...
  },
  {
    ...
    body: 'Every figure traces to a row someone entered in the source workbook. Nothing is projected, annualised, or smoothed. A partial current month stays partial rather than being scaled to a full one.',
  },
  {
  ...
  },
  {
    ...
    body: 'Access is enforced by Row Level Security in Postgres, not by hiding controls in the interface. Hiding a button is a courtesy; the policy is the boundary.',
  },
];

const METHOD = [
  ['Source', 'A Google Sheets workbook: the job book for revenue, time dedication for hours, the scope document for assumed dedication, and a master mapping that joins employee codes to names and regions.'],
  ['Join', 'Employee code from the master mapping links time dedication to people and regions. Client name links the job book, timesheets and scope document. Rows that fail to join appear in Evidence Exceptions rather than being discarded.'],
  ['Window', 'Every view is bounded by the From and To month selection. Totals never reach into future empty months.'],
  ['Currency', 'Regional figures convert to a single currency at the rate stated in the workbook. Conversion is applied once, at read time, never re-applied downstream.'],
];
  ...
  root.append(
    ...
    panel({
      ...
      body: el('div', { class: 'prose' }, [
        ...
        el('p', {
          text: 'It is deliberately read-only. Prism is not where the record is kept and not where it is corrected — the workbook remains the source of truth, and every page links back to it. What Prism adds is a consistent, inspectable reading of what is already there.',
        }),
      ]),
    }),
